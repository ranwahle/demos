...
const ANCESTRY_URL = '/ancestry';
let getDetails = person => request(`${ANCESTRY_URL}/${person.id}`);
let getDetailsForEach = ancestry => Promise.all(ancestry.map(getDetails));

request(ANCESTRY_URL)
    .then(res => getDetailsForEach(res.ANCESTRY_FILE))
    .then(onAncestryData)
    .catch(err => {
        displayError(`Something went wrong while loading ancestry data`);
