...
function render(state) {
  ...
  root.innerHTML = TodoApp(state);


  // We'll learn a better way to handle this later,
  // to register a task after render from within the component
...
function TodoApp(state) {
  return `
    <div class="column">
      <h1>Todos</h1>
      <main>
        <div class="InputContainer">${TodoInput(state)}</div>
        <div class="ListContainer">${TodoList(state)}</div>
        <div class="FooterContainer">${TodoFooter(state)}</div>
      </main>
    </div>
  `;
}
